...
export function analyzeMileage(mileageData = [], recentTrips = []) {
  ...
  for (const row of mileageData) {
    ...
    const { nextType, milesUntilService } = nextService(sinceOil, sinceBrakes, sinceTires, oilInterval);
 ...
 */
function nextService(sinceOil, sinceBrakes, sinceTires, oilInterval = OIL_CHANGE_MILES_DEFAULT) {
  const candidates = [
